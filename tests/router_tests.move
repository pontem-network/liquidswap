...
module liquidswap::router_tests {
    ...
    fun register_stable_pool_with_liquidity(x_val: u64, y_val: u64): (signer, signer) {
        ...
        router::register_pool<USDC, USDT>(&lp_owner, 1);
        ...
        if (x_val != 0 && y_val != 0) {
            ...
            let usdt_coins = test_coins::mint<USDT>(&coin_admin, y_val);
            let lp_coins =
                liquidity_pool::mint<USDC, USDT>(lp_owner_addr, usdc_coins, usdt_coins);
    ...
    fun test_add_initial_liquidity() {
        ...
        let pool_addr = signer::address_of(&lp_owner);
    ...
    fun test_add_liquidity_to_fail_with_insufficient_y_coins() {
        ...
        let (coin_y, coin_x, lp_coins) = router::add_liquidity<USDT, BTC>(
            pool_addr,
            usdt_coins,
            9000,
            btc_coins,
    ...
    fun test_swap_exact_coin_for_coin() {
        ...
        let usdt_amount_out = router::get_amount_out<BTC, USDT>(lp_owner_addr, btc_coins_swap_val);
    ...
    fun test_swap_exact_coin_for_coin_1() {
        let (coin_admin, lp_owner) = register_pool_with_liquidity(1230000000, 147600000000);

        let lp_owner_addr = signer::address_of(&lp_owner);
        let btc_to_swap_val = 572123800;
        let btc_coins_to_swap = test_coins::mint<BTC>(&coin_admin, btc_to_swap_val);

        let usdt_to_get_val = router::get_amount_out<BTC, USDT>(lp_owner_addr, btc_to_swap_val);
    ...
    fun test_swap_exact_coin_for_coin_2() {
        let (coin_admin, lp_owner) = register_pool_with_liquidity(10000000000, 2800000000000);

        let lp_owner_addr = signer::address_of(&lp_owner);
        let usdt_to_swap_val = 257817560;
        let usdt_to_swap = test_coins::mint<USDT>(&coin_admin, usdt_to_swap_val);

        let btc_to_get_val = router::get_amount_out<USDT, BTC>(lp_owner_addr, usdt_to_swap_val);
    ...
    fun test_swap_coin_for_exact_coin_1() {
        let (coin_admin, lp_owner) = register_pool_with_liquidity(50000000000, 13500000000000);

        let lp_owner_addr = signer::address_of(&lp_owner);

        let usdt_coins_to_get = 5292719411;
        let btc_coins_to_swap_val = router::get_amount_in<BTC, USDT>(lp_owner_addr, usdt_coins_to_get);
    ...
    fun test_swap_coin_for_exact_coin_2() {
        ...
        let usdt_coins_to_swap_val = router::get_amount_in<USDT, BTC>(lp_owner_addr, btc_coins_to_get);
    ...
    fun test_swap_coin_for_exact_coin_router_check_fails() {
        ...
        let btc_coins_to_swap = test_coins::mint<BTC>(&coin_admin, btc_coins_to_swap_val);
        let usdt_to_get = router::get_amount_out<BTC, USDT>(lp_owner_addr, btc_coins_to_swap_val) + 1;
    ...
    fun test_returned_usdt_proportially_decrease_for_big_swaps() {
        ...
        let (btc_reserve, usdt_reserve) = router::get_reserves_size<BTC, USDT>(lp_owner_addr);
        assert!(btc_reserve == 301, 1);
        assert!(usdt_reserve == 3396, 2);
        assert!(router::current_price<USDT, BTC>(lp_owner_addr) == 11, 3);
    ...
    fun test_pool_exists() {
        ...
        assert!(router::pool_exists_at<BTC, USDT>(signer::address_of(&lp_owner)), 0);
        assert!(router::pool_exists_at<USDT, BTC>(signer::address_of(&lp_owner)), 1);
    }
    ...
    fun test_stable_curve_swap_exact() {
        ...
        assert!(router::get_curve_type<USDC, USDT>(lp_owner_addr) == 1, 0);

        let usdc_to_swap_val = 1258044;

        let usdc_to_swap = test_coins::mint<USDC>(&coin_admin, usdc_to_swap_val);
        let usdt_to_get = router::get_amount_out<USDC, USDT>(lp_owner_addr, usdc_to_swap_val);
        ...
        coin::register<USDT>(&lp_owner);
        coin::deposit(lp_owner_addr, usdt_swapped);
    }
    ...
    fun test_stable_curve_swap_exact_vice_versa() {
        ...
        let usdc_to_get_val = router::get_amount_out<USDT, USDC>(lp_owner_addr, usdt_to_swap_val);
        let usdt_to_swap = test_coins::mint<USDT>(&coin_admin, usdt_to_swap_val);
        ...
        coin::deposit(lp_owner_addr, usdc_swapped);
    }
    ...
    fun test_stable_curve_exact_swap() {
        ...
        let usdt_to_swap_val = router::get_amount_in<USDT, USDC>(lp_owner_addr, usdc_to_get_val);
        ...
        coin::deposit(lp_owner_addr, usdt_reminder);
        coin::deposit(lp_owner_addr, usdc_swapped);
    }
    ...
    fun test_stable_curve_exact_swap_vice_versa() {
        ...
        let usdc_to_swap_val = router::get_amount_in<USDC, USDT>(lp_owner_addr, usdt_to_get_val);
        ...
        coin::deposit(lp_owner_addr, usdc_reminder);
        coin::deposit(lp_owner_addr, usdt_swapped);
    }

    #[test]
    fun test_stable_get_amount_in() {
        let (_, lp_owner) = register_stable_pool_with_liquidity(15000000000, 1500000000000);

        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_in = router::get_amount_in<USDC, USDT>(lp_owner_addr, 67279092);
        assert!(amount_in == 674816, 0);
    }

    #[test]
    fun test_get_amount_in() {
        let (_, lp_owner) = register_pool_with_liquidity(10828583259, 2764800200409);

        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_in = router::get_amount_in<USDT, BTC>(lp_owner_addr, 158202011);
    ...
    fun test_get_amount_in_1() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_in = router::get_amount_in<BTC, USDT>(lp_owner_addr, 28253021000);
    ...
    fun test_get_amount_in_2() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_in = router::get_amount_in<USDT, BTC>(lp_owner_addr, 1);
        assert!(amount_in == 257, 0);
    }

    #[test]
    fun test_stable_get_amount_in_1() {
        let (_, lp_owner) = register_stable_pool_with_liquidity(20000000000, 1000000000000);
    ...
    fun test_stable_get_amount_in_2() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_in = router::get_amount_in<USDC, USDT>(lp_owner_addr, 15000);
    ...
    fun test_stable_get_amount_in_3() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_in = router::get_amount_in<USDT, USDC>(lp_owner_addr, 158282982);
    ...
    fun test_stable_get_amount_in_4() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_in = router::get_amount_in<USDT, USDC>(lp_owner_addr, 1);
        assert!(amount_in == 102, 0);
    }

    #[test]
    fun test_stable_get_amount_in_5() {
        let (_, lp_owner) = register_stable_pool_with_liquidity(2930000000000, 293000000000000);

        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_in = router::get_amount_in<USDT, USDC>(lp_owner_addr, 57212828231);
    ...
    fun test_stable_get_amount_out() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_out = router::get_amount_out<USDC, USDT>(lp_owner_addr, 674816);
        assert!(amount_out == 67279199, 0);
    }

    #[test]
    fun test_get_amount_out() {
        let (_, lp_owner) = register_pool_with_liquidity(18000000000, 4680000000000);

        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_out = router::get_amount_out<USDT, BTC>(lp_owner_addr, 1500000000);
    ...
    fun test_get_amount_out_1() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_out = router::get_amount_out<BTC, USDT>(lp_owner_addr, 100000000);
    ...
    fun test_get_amount_out_2() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_out = router::get_amount_out<BTC, USDT>(lp_owner_addr, 1);
        assert!(amount_out == 259, 0);
    }

    #[test]
    fun test_stable_get_amount_out_1() {
        let (_, lp_owner) = register_stable_pool_with_liquidity(25000000000, 1500000000000);

        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_out = router::get_amount_out<USDC, USDT>(lp_owner_addr, 323859);
    ...
    fun test_stable_get_amount_out_2() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_out = router::get_amount_out<USDC, USDT>(lp_owner_addr, 58201);
    ...
    fun test_stable_get_amount_out_3() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_out = router::get_amount_out<USDT, USDC>(lp_owner_addr, 15000);
    ...
    fun test_stable_get_amount_out_4() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_out = router::get_amount_out<USDT, USDC>(lp_owner_addr, 1);
    ...
    fun test_stable_get_amount_out_5() {
        ...
        let lp_owner_addr = signer::address_of(&lp_owner);
        let amount_out = router::get_amount_out<USDT, USDC>(lp_owner_addr, 572123482812);
    ...
    fun test_get_curve_type_stables() {
        ...
        let lp_owner_address = signer::address_of(&lp_owner);
        assert!(router::get_curve_type<USDC, USDT>(lp_owner_address) == 1, 0);
        assert!(router::get_curve_type<USDT, USDC>(lp_owner_address) == 1, 1);
    }
    ...
    fun test_get_curve_type_uncorrelated() {
        ...
        let lp_owner_address = signer::address_of(&lp_owner);
        assert!(router::get_curve_type<BTC, USDT>(lp_owner_address) == 2, 0);
        assert!(router::get_curve_type<USDT, BTC>(lp_owner_address) == 2, 1);
    }
    ...
    fun test_get_decimals_scales_stables() {
        ...
        let lp_owner_address = signer::address_of(&lp_owner);
        let (x, y) = router::get_decimals_scales<USDC, USDT>(lp_owner_address);
    ...
    fun test_get_decimals_scales_uncorrelated() {
        ...
        let lp_owner_address = signer::address_of(&lp_owner);
        let (x, y) = router::get_decimals_scales<BTC, USDT>(lp_owner_address);
    ...
    fun test_stable_swap_coin_for_coin_unchecked() {
        let (coin_admin, lp_owner) = register_stable_pool_with_liquidity(150000000, 15000000000);

        let lp_owner_addr = signer::address_of(&lp_owner);
        let usdc_coins_swap_val = 100;
        let usdc_coins_to_swap = test_coins::mint<USDC>(&coin_admin, usdc_coins_swap_val);
        let usdt_amount_out = router::get_amount_out<USDC, USDT>(lp_owner_addr, usdc_coins_swap_val);
    ...
    fun test_stable_swap_coin_for_coin_unchecked_reverse() {
        ...
        let usdt_to_swap = test_coins::mint<USDT>(&coin_admin, usdt_to_swap_val);
        let usdt_amount_out = router::get_amount_out<USDT, USDC>(lp_owner_addr, usdt_to_swap_val);
    ...
    fun test_swap_coin_for_coin_unchecked_1() {
        ...
        let usdt_to_get_val = router::get_amount_out<BTC, USDT>(lp_owner_addr, btc_to_swap_val) - 1;
    ...
    fun test_swap_coin_for_coin_unchecked_2() {
        ...
        let btc_to_get_val = router::get_amount_out<USDT, BTC>(lp_owner_addr, usdt_to_swap_val) - 134567;
    ...
    fun test_stable_fail_if_price_fell_behind_threshold_unchecked_2() {
        ...
        let usdt_coins_to_swap_val = router::get_amount_in<USDT, USDC>(lp_owner_addr, usdc_coins_to_get);
    ...
    fun test_swap_coin_for_coin_unchecked_fails() {
        ...
        let btc_to_get_val = router::get_amount_out<USDT, BTC>(lp_owner_addr, usdt_to_swap_val) + 1;
    ...
    fun test_calc_optimal_coin_values() {
        ...
        let (_, lp_owner) = register_pool_with_liquidity(10000000000, 2800000000000);
        ...
        let (x_value, y_value) = router::calc_optimal_coin_values<BTC, USDT>(lp_owner_address, x_desired, y_desired, 0, 0);
    ...
    fun test_calc_optimal_coin_values_1() {
        ...
        let (_x_value, _y_value) = router::calc_optimal_coin_values<BTC, USDT>(lp_owner_address, x_desired, y_desired, 5000000000, 0);
    }

    #[test]
    fun test_calc_optimal_coin_values_3() {
        // 100 BTC, 28000 USDT
        let (_, lp_owner) = register_pool_with_liquidity(10000000000, 28000000000);
        ...
        let (x_res, y_res) = router::get_reserves_size<BTC, USDT>(lp_owner_address);
    ...
    fun test_calc_optimal_coin_values_4() {
        ...
        let (_x_value, _y_value) = router::calc_optimal_coin_values<BTC, USDT>(lp_owner_address, x_desired, y_desired, 0, 2800000000);
    }
}
