module liquidswap::dao_storage {
    ...
    struct Storage<phantom X, phantom Y> has key {
        coin_x: Coin<X>,
        coin_y: Coin<Y>
    }
    ...
    public(friend) fun register<X, Y>(owner: &signer) {
        let storage = Storage<X, Y> { coin_x: coin::zero<X>(), coin_y: coin::zero<Y>() };
    ...
    public(friend) fun deposit<X, Y>(pool_addr: address, coin_x: Coin<X>, coin_y: Coin<Y>) acquires Storage, EventsStore {
        assert!(exists<Storage<X, Y>>(pool_addr), ERR_NOT_REGISTERED);

        let x_val = coin::value(&coin_x);
        let y_val = coin::value(&coin_y);
        let storage = borrow_global_mut<Storage<X, Y>>(pool_addr);
    ...
    public fun withdraw<X, Y>(dao_admin_acc: &signer, pool_addr: address, x_val: u64, y_val: u64): (Coin<X>, Coin<Y>)
    acquires Storage, EventsStore {
        ...
        event::emit_event(
            ...
            CoinWithdrawnEvent<X, Y> { x_val, y_val }
        );
    ...
    #[test_only]
    public fun get_storage_size<X, Y>(pool_addr: address): (u64, u64) acquires Storage {
        let storage = borrow_global<Storage<X, Y>>(pool_addr);
    ...
    #[test_only]
    public fun register_for_test<X, Y>(owner: &signer) {
        register<X, Y>(owner);
    }

    #[test_only]
    public fun deposit_for_test<X, Y>(
        pool_addr: address,
        coin_x: Coin<X>,
        coin_y: Coin<Y>
    ) acquires Storage, EventsStore {
        deposit<X, Y>(pool_addr, coin_x, coin_y);
    }

    // Events

    struct EventsStore<phantom X, phantom Y> has key {
        storage_registered_handle: event::EventHandle<StorageCreatedEvent<X, Y>>,
        coin_deposited_handle: event::EventHandle<CoinDepositedEvent<X, Y>>,
        coin_withdrawn_handle: event::EventHandle<CoinWithdrawnEvent<X, Y>>,
    }

    struct StorageCreatedEvent<phantom X, phantom Y> has store, drop {}

    struct CoinDepositedEvent<phantom X, phantom Y> has store, drop {
        x_val: u64,
        y_val: u64,
    }

    struct CoinWithdrawnEvent<phantom X, phantom Y> has store, drop {
        x_val: u64,
        y_val: u64,
    }
}
