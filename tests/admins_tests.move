...
module liquidswap::admins_tests {
    ...
    fun test_emergency_admin() {
        ...
        assert!(admins::get_emergency_admin() == @emergency_admin, 0);
        ...
        assert!(admins::get_emergency_admin() == @test_coin_admin, 1);

        let test_coin_admin = account::create_account_for_test(@test_coin_admin);
        admins::set_emergency_admin(&test_coin_admin, @emergency_admin);
        assert!(admins::get_emergency_admin() == @emergency_admin, 2);
    }
    ...
    fun test_fee_admin() {
        ...
        assert!(admins::get_fee_admin() == @fee_admin, 0);

        let fee_admin = account::create_account_for_test(@fee_admin);
        admins::set_fee_admin(&fee_admin, @test_coin_admin);
        assert!(admins::get_fee_admin() == @test_coin_admin, 1);

        let test_coin_admin = account::create_account_for_test(@test_coin_admin);
        admins::set_fee_admin(&test_coin_admin, @fee_admin);
        assert!(admins::get_fee_admin() == @fee_admin, 2);
    }

    #[test]
    #[expected_failure(abort_code = 300)]
    fun test_get_fee_admin_fail_if_config_is_not_initialized() {
        admins::get_fee_admin();
    }
    ...
    fun test_set_fee_admin_fail_if_user_is_not_fee_admin() {
        ...
        let fee_admin = account::create_account_for_test(@fee_admin);
        admins::set_fee_admin(&fee_admin, @test_coin_admin);
        assert!(admins::get_fee_admin() == @test_coin_admin, 0);
