...
module AptosSwap::Router {
    ...
    public fun add_liquidity_inner<X: store, Y: store, LP>(pool_addr: address, coin_x: Coin<X>, coin_y: Coin<Y>): Coin<LP> {
        if (CoinHelper::is_sorted<X, Y>()) {
    ...
    public fun remove_liquidity<X: store, Y: store, LP>(
        pool_addr: address,
        lp_coins: Coin<LP>,
        min_x_out_val: u64,
        min_y_out_val: u64
    ...
    public fun swap_exact_coin_for_coin<X: store, Y: store, LP>(
        pool_addr: address,
        coin_in: Coin<X>,
    ...
    public fun swap_coin_for_exact_coin<X: store, Y: store, LP>(
        pool_addr: address,
        coin_max_in: Coin<X>,
        coin_out_val: u64,
    ): (Coin<X>, Coin<Y>) {
    ...
    public fun calc_optimal_coin_values<X: store, Y: store, LP>(
        pool_addr: address,
        x_desired: u64,
        y_desired: u64,
        x_min: u64,
    ...
    public fun get_reserves_for_lp_coins<X: store, Y: store, LP>(
        pool_addr: address,
        lp_to_burn_val: u64
    ): (u64, u64) {
    ...
    public fun get_coin_out_with_fees(coin_in_val: u64, reserve_in_size: u64, reserve_out_size: u64): u64 {
    ...
    /// * `reserve_in_size` - reserves of coin we are going to swap.
    /// * `reserve_out_size` - reserves of coin we are going to get.
    public fun get_coin_in_with_fees(coin_out_val: u64, reserve_out_size: u64, reserve_in_size: u64): u64 {
        let (fee_pct, fee_scale) = LiquidityPool::get_fees_config();
