module liquidswap::gauge {
    ...
    use gauge_admin::iter_table::{Self, IterTable};
    use liquidswap::lamm::LAMM;
    use liquidswap::liquidity_pool;
    use liquidswap::ve::{Self, VE_NFT};
    use std::vector;

    struct PoolVotingId has copy, drop, store {
    ...
    struct Votes has key {
        ...
        // pool_id -> total voted
        total_voted_by_pool: Table<PoolVotingId, u64>,
        // pool_id -> table<token_id, stake>
        items: IterTable<PoolVotingId, IterTable<u64, u64>>,
        // token_id -> coins
        rewards: Table<u64, Coin<LAMM>>
    }

    public fun initialize(gauge_admin: &signer) {
        move_to(gauge_admin, Votes {
            total_vote: 0,
    ...
    public fun vote_for_liquidity_pool<X, Y, LP>(pool_addr: address, ve_nft: &VE_NFT, vote_stake: u64) acquires Votes {
        let pool_voting_id = liquidity_pool_voting_id<X, Y, LP>(pool_addr);
        let ve_token_id = ve::get_nft_id(ve_nft);
        let ve_total_stake = ve::get_nft_staked_value(ve_nft);

        let votes = borrow_global_mut<Votes>(@gauge_admin);
    ...
    public fun distribute_rewards(rewards: Coin<LAMM>) acquires Votes {
        let votes = borrow_global_mut<Votes>(@gauge_admin);
    ...
    fun liquidity_pool_voting_id<X, Y, LP>(pool_addr: address): PoolVotingId {
