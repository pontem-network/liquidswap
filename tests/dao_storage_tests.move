#[test_only]
module liquidswap::dao_storage_tests {
    use std::signer;

    use aptos_framework::account;
    use aptos_framework::coin;

    use liquidswap_lp::lp_coin::LP;
    use test_coins::coins::{Self, BTC, USDT};

    use liquidswap::curves::Uncorrelated;
    use liquidswap::dao_storage;
    use liquidswap::global_config;
    use liquidswap::liquidity_pool;
    use liquidswap::router;
    use liquidswap::test_pool;

    fun dao_admin(): signer {
        account::create_account_for_test(@dao_admin)
    }

    #[test]
    fun test_register() {
        let (_, lp_user) = test_pool::setup_btc_usdt_coins_and_lp_user();

        dao_storage::register_for_test<BTC, USDT, Uncorrelated>(&lp_user);

        let (x_val, y_val) = dao_storage::get_storage_size<BTC, USDT, Uncorrelated>(signer::address_of(&lp_user));
        assert!(x_val == 0, 0);
        assert!(y_val == 0, 1);
    }

    #[test]
    fun test_deposit() {
        let (coin_admin, lp_user) = test_pool::setup_btc_usdt_coins_and_lp_user();

        dao_storage::register_for_test<BTC, USDT, Uncorrelated>(&lp_user);

        let lp_user_addr = signer::address_of(&lp_user);
        let btc_coins = coins::mint<BTC>(&coin_admin, 100000000);
        let usdt_coins = coins::mint<USDT>(&coin_admin, 1000000);

        let (x_val, y_val) = dao_storage::get_storage_size<BTC, USDT, Uncorrelated>(signer::address_of(&lp_user));
        assert!(x_val == 0, 0);
        assert!(y_val == 0, 1);

        dao_storage::deposit_for_test<BTC, USDT, Uncorrelated>(lp_user_addr, btc_coins, usdt_coins);
        (x_val, y_val) = dao_storage::get_storage_size<BTC, USDT, Uncorrelated>(signer::address_of(&lp_user));
        assert!(x_val == 100000000, 2);
        assert!(y_val == 1000000, 3);
    }

    #[test]
    #[expected_failure(abort_code = 401)]
    fun test_deposit_fail_if_not_registered() {
        let (coin_admin, lp_user) = test_pool::setup_btc_usdt_coins_and_lp_user();

        let lp_user_addr = signer::address_of(&lp_user);
        let btc_coins = coins::mint<BTC>(&coin_admin, 100000000);
        let usdt_coins = coins::mint<USDT>(&coin_admin, 1000000);

        dao_storage::deposit_for_test<BTC, USDT, Uncorrelated>(lp_user_addr, btc_coins, usdt_coins);
    }

    #[test]
    fun test_withdraw() {
        let (coin_admin, lp_user) = test_pool::setup_btc_usdt_coins_and_lp_user();

        dao_storage::register_for_test<BTC, USDT, Uncorrelated>(&lp_user);

        let lp_user_addr = signer::address_of(&lp_user);
        let btc_coins = coins::mint<BTC>(&coin_admin, 100000000);
        let usdt_coins = coins::mint<USDT>(&coin_admin, 1000000);

        dao_storage::deposit_for_test<BTC, USDT, Uncorrelated>(lp_user_addr, btc_coins, usdt_coins);

        let dao_admin = dao_admin();
        let (x, y) =
            dao_storage::withdraw<BTC, USDT, Uncorrelated>(&dao_admin, lp_user_addr, 100000000, 0);
        assert!(coin::value(&x) == 100000000, 0);
        assert!(coin::value(&y) == 0, 1);

        let (x_val, y_val) = dao_storage::get_storage_size<BTC, USDT, Uncorrelated>(signer::address_of(&lp_user));
        assert!(x_val == 0, 2);
        assert!(y_val == 1000000, 3);

        global_config::set_dao_admin(&dao_admin, signer::address_of(&coin_admin));
        let (x0, y0) =
            dao_storage::withdraw<BTC, USDT, Uncorrelated>(&coin_admin, lp_user_addr, 0, 1000000);
        assert!(coin::value(&x0) == 0, 4);
        assert!(coin::value(&y0) == 1000000, 5);

        coins::burn(&coin_admin, x);
        coins::burn(&coin_admin, y);
        coins::burn(&coin_admin, x0);
        coins::burn(&coin_admin, y0);
    }

    #[test]
    #[expected_failure(abort_code = 65542)]
    fun test_withdraw_fail_if_more_deposited() {
        let (coin_admin, lp_user) = test_pool::setup_btc_usdt_coins_and_lp_user();

        dao_storage::register_for_test<BTC, USDT, Uncorrelated>(&lp_user);

        let lp_user_addr = signer::address_of(&lp_user);
        let btc_coins = coins::mint<BTC>(&coin_admin, 100000000);
        let usdt_coins = coins::mint<USDT>(&coin_admin, 1000000);

        dao_storage::deposit_for_test<BTC, USDT, Uncorrelated>(lp_user_addr, btc_coins, usdt_coins);

        let dao_admin = dao_admin();
        let (x, y) =
            dao_storage::withdraw<BTC, USDT, Uncorrelated>(&dao_admin, lp_user_addr, 200000000, 0);

        coins::burn(&coin_admin, x);
        coins::burn(&coin_admin, y);
    }

    #[test]
    #[expected_failure(abort_code = 402)]
    fun test_withdraw_fail_if_not_dao_admin() {
        let (coin_admin, lp_user) = test_pool::setup_btc_usdt_coins_and_lp_user();

        dao_storage::register_for_test<BTC, USDT, Uncorrelated>(&lp_user);

        let lp_user_addr = signer::address_of(&lp_user);
        let btc_coins = coins::mint<BTC>(&coin_admin, 100000000);
        let usdt_coins = coins::mint<USDT>(&coin_admin, 1000000);

        dao_storage::deposit_for_test<BTC, USDT, Uncorrelated>(lp_user_addr, btc_coins, usdt_coins);

        let not_dao_admin = account::create_account_for_test(@0xca);
        let (x, y) =
            dao_storage::withdraw<BTC, USDT, Uncorrelated>(&not_dao_admin, lp_user_addr, 100000000, 0);

        coins::burn(&coin_admin, x);
        coins::burn(&coin_admin, y);
    }

    #[test]
    fun test_split_third_of_fees_into_dao_storage_account() {
        let (coin_admin, lp_user) = test_pool::setup_btc_usdt_coins_and_lp_user();

        // 0.3% fee
        router::register_pool<BTC, USDT, Uncorrelated>(&lp_user);

        let btc_coins = coins::mint<BTC>(&coin_admin, 100000);
        let usdt_coins = coins::mint<USDT>(&coin_admin, 100000);

        let lp_coins =
            liquidity_pool::mint<BTC, USDT, Uncorrelated>(btc_coins, usdt_coins);
        coin::register<LP<BTC, USDT, Uncorrelated>>(&lp_user);
        coin::deposit(signer::address_of(&lp_user), lp_coins);

        let btc_coins_to_exchange = coins::mint<BTC>(&coin_admin, 1000);
        let (zero, usdt_coins) =
            liquidity_pool::swap<BTC, USDT, Uncorrelated>(
                btc_coins_to_exchange, 0,
                coin::zero<USDT>(), 960
            );

        let (x_res, y_res) = liquidity_pool::get_reserves_size<BTC, USDT, Uncorrelated>();
        assert!(x_res == 100999, 2);
        assert!(y_res == 99040, 3);

        let (dao_x, dao_y) = dao_storage::get_storage_size<BTC, USDT, Uncorrelated>(@liquidswap_pool_account);
        assert!(dao_x == 1, 4);
        assert!(dao_y == 0, 5);

        let dao_admin = dao_admin();
        let (x, y) =
            dao_storage::withdraw<BTC, USDT, Uncorrelated>(&dao_admin, @liquidswap_pool_account, 1, 0);
        assert!(coin::value(&x) == 1, 6);
        assert!(coin::value(&y) == 0, 7);

        coins::burn(&coin_admin, x);
        coins::burn(&coin_admin, y);

        coin::destroy_zero(zero);
        coins::burn(&coin_admin, usdt_coins);
    }
}
