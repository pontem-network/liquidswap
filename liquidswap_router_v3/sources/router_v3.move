...
module liquidswap::router_v3 {
    ...
    fun get_coin_out_with_fees<X, Y, Curve>(
        ...
        if (curves::is_stable<Curve>()) {
            let coin_in_val_after_fees = math_v2::mul_div_up_u128(
                (coin_in as u128),
                (fee_multiplier as u128),
                (fee_scale as u128)
            );

            stable_curve_v2::coin_out(
    ...
    fun get_coin_in_with_fees<X, Y, Curve>(
        ...
        if (curves::is_stable<Curve>()) {
            let coin_in = stable_curve_v2::coin_in(
                coin_out,
                scale_out,
                scale_in,
                reserve_out_u128,
            ...
            math_v2::mul_div_up_u128((coin_in as u128), (fee_scale as u128), (fee_multiplier as u128))
        } else if (curves::is_uncorrelated<Curve>()) {
            let new_reserves_out = (reserve_out_u128 - coin_out_u128) * (fee_multiplier as u128);

            // coin_out * reserve_in * fee_scale / new reserves out
            math_v2::mul_div_up_u128(coin_out_u128, reserve_in_u128 * (fee_scale as u128), new_reserves_out)
        } else {
            abort ERR_UNREACHABLE
        }
    }
