...
module liquidswap::flashloan_tests {
    ...
    fun register_stable_pool_with_liquidity(x_val: u64, y_val: u64): (signer, signer, signer) {
        ...
        if (x_val != 0 && y_val != 0) {
            ...
            coin::deposit<LP<USDC, USDT, Stable>>(signer::address_of(&lp_user), lp_coins);
        };
    ...
    fun register_usdd_pool_with_liquidity(x_val: u64, y_val: u64): (signer, signer, signer) {
        ...
        if (x_val != 0 && y_val != 0) {
            ...
            coin::deposit<LP<USDD, USDT, Stable>>(signer::address_of(&lp_user), lp_coins);
        };
    ...
    fun test_flashloan_coins_with_normal_reserves_and_amount() {
        ...
        let btc_coins_to_exchange = coins::mint<BTC>(&coin_admin, 1000000);
    ...
    fun test_flashloan_coins_with_normal_reserves_and_max_amount() {
        ...
        let btc_coins_to_add = coins::mint<BTC>(&coin_admin, 303909);
        let usdt_coins_to_add = coins::mint<USDT>(&coin_admin, 83421050);
    ...
    fun test_flashloan_coins_with_min_reserves_and_normal_amount() {
        ...
        let btc_coins_to_exchange = coins::mint<BTC>(&coin_admin, 100);
        liquidity_pool::pay_flashloan(btc_coins_to_exchange, coin::zero<USDT>(), loan);
    ...
    fun test_flashloan_coins_with_min_reserves_and_max_amount() {
        ...
        let btc_coins_to_add = coins::mint<BTC>(&coin_admin, 4);
        let usdt_coins_to_add = coins::mint<USDT>(&coin_admin, 3);
        coin::merge(&mut btc_coins, btc_coins_to_add);
        coin::merge(&mut usdt_coins, usdt_coins_to_add);
        liquidity_pool::pay_flashloan(btc_coins, usdt_coins, loan);
    ...
    fun test_flashloan_coins_with_max_reserves_and_normal_amount() {
        ...
        let btc_coins_to_exchange = coins::mint<BTC>(&coin_admin, 10000000000);
        let (zero, usdt_coins, loan) =
    ...
    fun test_flashloan_coins_from_stable_pool_with_normal_reserves_and_amount() {
        let (coin_admin, coin_extended_admin, _) =
            register_usdd_pool_with_liquidity(15000000000, 1500000000000);

        let (zero, usdt_coins, loan) =
            liquidity_pool::flashloan<USDD, USDT, Stable>(0, 99699999);
        assert!(coin::value(&usdt_coins) == 99699999, 1);

        let usdc_coins_to_exchange = coins::mint<USDD>(&coin_extended_admin, 1000000);
    ...
    fun test_flashloan_coins_from_stable_pool_with_normal_reserves_and_min_amount() {
        let (coin_admin, coin_extended_admin, _) =
            register_usdd_pool_with_liquidity(15000999000, 1499900300001);

        let (zero, usdt_coins, loan) =
            liquidity_pool::flashloan<USDD, USDT, Stable>(0, 99);
    ...
    fun test_flashloan_coins_from_stable_pool_with_min_reserves_and_normal_amount() {
        let (coin_admin, coin_extended_admin, _) = register_usdd_pool_with_liquidity(1001, 1001);

        let (zero, usdt_coins, loan) =
            liquidity_pool::flashloan<USDD, USDT, Stable>(0, 90);
        assert!(coin::value(&usdt_coins) == 90, 1);

        let usdc_coins_to_exchange = coins::mint<USDD>(&coin_extended_admin, 33);
        liquidity_pool::pay_flashloan(usdc_coins_to_exchange, coin::zero<USDT>(), loan);
    ...
    fun test_flashloan_coins_from_stable_pool_with_min_reserves_and_min_amount() {
        let (coin_admin, coin_extended_admin, _) = register_stable_pool_with_liquidity(1001, 1001);

        let (zero, usdt_coins, loan) =
            liquidity_pool::flashloan<USDC, USDT, Stable>(0, 1);
        assert!(coin::value(&usdt_coins) == 1, 1);

        let usdc_coins_to_exchange = coins::mint<USDC>(&coin_extended_admin, 2);
        liquidity_pool::pay_flashloan(usdc_coins_to_exchange, coin::zero<USDT>(), loan);
    ...
    fun test_flashloan_coins_from_stable_pool_with_min_reserves_and_max_amount() {
        ...
        let usdc_coins_to_add = coins::mint<USDC>(&coin_extended_admin, 4);
        let usdt_coins_to_add = coins::mint<USDT>(&coin_admin, 3);
    ...
    fun test_flashloan_coins_from_stable_pool_with_big_reserves_and_normal_amount() {
        let (coin_admin, coin_extended_admin, _) =
            register_usdd_pool_with_liquidity(2930000000000, 293000000000000);

        let (zero, usdt_coins, loan) =
            liquidity_pool::flashloan<USDD, USDT, Stable>(0, 996999980359);
        assert!(coin::value(&usdt_coins) == 996999980359, 1);

        let usdc_coins_to_exchange = coins::mint<USDD>(&coin_extended_admin, 10000000000);
        liquidity_pool::pay_flashloan(usdc_coins_to_exchange, coin::zero<USDT>(), loan);
    ...
    fun test_flashloan_coins_from_stable_pool_with_big_reserves_and_min_amount() {
        let (coin_admin, coin_extended_admin, _) = register_usdd_pool_with_liquidity(2930000000000, 293000000000000);
    ...
    fun test_fail_if_pay_less_flashloaned_coins() {
        ...
        let btc_coins_to_exchange = coins::mint<BTC>(&coin_admin, 999999);
        liquidity_pool::pay_flashloan(btc_coins_to_exchange, coin::zero<USDT>(), loan);
    ...
    fun test_fail_if_mint_when_pool_is_locked() {
        let (coin_admin, lp_user) = register_pool_with_liquidity(100000000, 28000000000);
        ...
        let btc_coins_mint = coins::mint<BTC>(&coin_admin, 1000000);
        let usdt_coins_mint = coins::mint<USDT>(&coin_admin, 280000000);
        let lp_coins_mint =
            liquidity_pool::mint<BTC, USDT, Uncorrelated>(btc_coins_mint, usdt_coins_mint);
        coin::deposit(signer::address_of(&lp_user), lp_coins_mint);
    ...
    fun test_fail_if_swap_when_pool_is_locked() {
        ...
        coins::burn(&coin_admin, usdt_coins_swap);
    ...
    fun test_fail_if_burn_when_pool_is_locked() {
        ...
        let lp_coins = coin::withdraw<LP<BTC, USDT, Uncorrelated>>(&lp_user, 16733190);
