...
module liquidswap::liquidity_pool_tests {
    ...
    fun test_create_empty_pool() {
        ...
        let (x_res_val, y_res_val) =
            liquidity_pool::get_reserves_size<BTC, USDT>(lp_owner_addr);
        assert!(x_res_val == 0, 0);
        assert!(y_res_val == 0, 1);

        let (x_price, y_price, _) =
            liquidity_pool::get_cumulative_prices<BTC, USDT>(lp_owner_addr);
        assert!(x_price == 0, 2);
        assert!(y_price == 0, 3);

        // Check created LP.

        let curve_type = liquidity_pool::get_curve_type<BTC, USDT>(lp_owner_addr);
        ...
        let (x_cum_price, y_cum_price, ts) = liquidity_pool::get_cumulative_prices<BTC, USDT>(lp_owner_addr);
        assert!(x_cum_price == 0, 10);
        assert!(y_cum_price == 0, 11);
        assert!(ts == 0, 12);

        // Check if it's locked.
        assert!(!liquidity_pool::is_pool_locked<BTC, USDT>(lp_owner_addr), 13);
    }
    ...
    fun test_create_empty_pool_stable() {
        ...
        // Check scales.
        let (x_scale, y_scale) = liquidity_pool::get_decimals_scales<BTC, USDT>(lp_owner_addr);
        ...
        let (x_cumm_price, y_cumm_price, ts) = liquidity_pool::get_cumulative_prices<BTC, USDT>(lp_owner_addr);
        assert!(x_cumm_price == 0, 9);
        assert!(y_cumm_price == 0, 10);
        assert!(ts == 0, 11);

        // Check if it's locked.
        assert!(!liquidity_pool::is_pool_locked<BTC, USDT>(lp_owner_addr), 12);
    }
    ...
    fun test_fail_if_coin_generics_provided_in_the_wrong_order() {
        ...
        let (_x_price, _y_price, _) =
            liquidity_pool::get_cumulative_prices<USDT, BTC>(lp_owner_addr);
    }
    ...
    fun test_add_liquidity_to_empty_pool() {
        ...
        let lp_coins =
            liquidity_pool::mint<BTC, USDT>(lp_owner_addr, btc_liq, usdt_liq);
        ...
        let (x_res, y_res) = liquidity_pool::get_reserves_size<BTC, USDT>(lp_owner_addr);
        assert!(x_res == btc_liq_val, 2);
        assert!(y_res == usdt_liq_val, 3);

        let (x_price, y_price, ts) = liquidity_pool::get_cumulative_prices<BTC, USDT>(lp_owner_addr);
        assert!(x_price == 0, 4);
        assert!(y_price == 0, 5);
    ...
    fun test_add_liquidity_zero() {
        ...
        let lp_coins_zero = liquidity_pool::mint<BTC, USDT>(lp_owner_addr, coin::zero(), coin::zero());
    ...
    fun test_burn_liquidity() {
        ...
        let (btc_return, usdt_return) =
            liquidity_pool::burn<BTC, USDT>(lp_owner_addr, lp_coins);
    ...
    fun test_burn_liquidity_after_initial() {
        ...
        let (btc_return, usdt_return) =
            liquidity_pool::burn<BTC, USDT>(lp_owner_addr, lp_coins_initial);
        ...
        let (btc_return, usdt_return) =
            liquidity_pool::burn<BTC, USDT>(lp_owner_addr, lp_coins_user);
    ...
    fun test_swap_coins_with_stable_curve_type() {
        ...
        let (x_res, y_res) = liquidity_pool::get_reserves_size<USDC, USDT>(lp_owner_addr);
    ...
    fun test_get_reserves_emergency_fails(emergency_acc: signer) {
        let (_, lp_owner) = setup_btc_usdt_pool();

        emergency::pause(&emergency_acc);

        let (_, _) = liquidity_pool::get_reserves_size<BTC, USDT>(signer::address_of(&lp_owner));
    }
    ...
    fun test_get_cumulative_price_emergency_fails(emergency_acc: signer) {
        ...
        let (_, _, _) = liquidity_pool::get_cumulative_prices<BTC, USDT>(signer::address_of(&lp_owner));
    }
    ...
    fun test_pool_exists() {
        ...
        assert!(liquidity_pool::pool_exists_at<BTC, USDT>(signer::address_of(&lp_owner)), 0);
        assert!(!liquidity_pool::pool_exists_at<USDT, BTC>(signer::address_of(&lp_owner)), 1);
    }
    ...
    fun test_end_to_end() {
        ...
        let lp_coins_initial =
            liquidity_pool::mint<BTC, USDT>(lp_owner_addr, btc_coins_initial, usdt_coins_initial);
        ...
        let lp_coins_user =
            liquidity_pool::mint<BTC, USDT>(lp_owner_addr, btc_coins_user, usdt_coins_user);
