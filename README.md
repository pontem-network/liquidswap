...
let btc_coins_to_swap_val = router::get_amount_in<BTC, USDT, LP<BTC, USDT, Uncorrelated>>(pool_owner_addr, usdt_coins_to_get);
let btc_coins_to_swap = coin::withdraw<BTC>(account, btc_coins_to_swap_val);

let (coin_x, coin_y) = router::swap_coin_for_exact_coin<BTC, USDT, LP<BTC, USDT, Uncorrelated>>(
    pool_owner_addr,
    btc_coins_to_swap,
    usdt_coins_to_get
);
